...
def github_login():
    ...
    github_auth_url = (
        f"{GITHUB_AUTHORIZE_URL}?" f"client_id={client_id}&" f"redirect_uri={redirect_uri}&" f"scope={scope}"
    )
...
def github_callback():
    ...
    if not primary_email:
        try:
            ...
            primary_email = next(
                (item["email"] for item in email_data if item.get("primary") and item.get("verified")), None
            )
        except:
            pass

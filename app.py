...
def error_500(code):
    return render_template(
        "error.html", code="500 Internal Server Error!"), 500
    ...
    if request.method == "POST":
        ...
        if custom_alias:
            ...
            if not validate_alias(custom_alias):
                ...
                return redirect("/")
            check = db.execute(
                "SELECT clip_url from clips WHERE clip_url=?",
                custom_alias)
            if len(check) != 0:
            ...
            if remove_time == "never":
                ...
                try:
                    ...
                    remove_time = (
                        datetime.now() +
                        timedelta(
                            hours=hours)).strftime("%d-%m-%Y %H:%M:%S")
                except ValueError:
        ...
        if loginData()[0]:
            uid = db.execute(
                ...
                loginData()[1])[0]["id"]
            cid = db.execute(
                "SELECT id FROM clips WHERE clip_url=?",
                str(post_id))[0]["id"]
            db.execute(
                "INSERT INTO clipRef (userid, clipid) VALUES (?, ?)",
                int(uid),
                int(cid))
...
def clip(clip_url_id):
    ...
    if len(data) != 0:
        ...
        time_left = ""
        if remove_time:
            if datetime.strptime(remove_time,
                                 "%d-%m-%Y %H:%M:%S") < datetime.now():
                db.execute("DELETE FROM clips WHERE clip_url=?", clip_url_id)
                return render_template(
                    "error.html", code="This Clip was Expired.", dat=loginData()), 404
        ...
        return render_template(
            "error.html", code="That was not found on this server.", dat=loginData()), 404
    ...
    if len(data) != 0:
        ...
        if remove_time:
            if datetime.strptime(remove_time,
                                 "%d-%m-%Y %H:%M:%S") < datetime.now():
                db.execute("DELETE FROM clips WHERE clip_url=?", clip_url_id)
                return Response("This Clip was Expired.",
...
def delete(url_id):
    if loginData()[0]:
        ...
        if len(data) != 0:
            ...
            return render_template(
                "error.html", code="Cannot Delete this Clip!")
    return redirect("/login")
...
def download(url_id):
    data = db.execute(
        "SELECT clip_text, clip_name, clip_pwd FROM clips WHERE clip_url=? ",
        url_id)
    # HOT FIX
    if len(data) == 0:
        return render_template("error.html", code="404 Not Found!"), 404
...
def get_data():
    ...
    if clip_name and unlisted == "true":
        return jsonify(
            {"Message": "To Search, enter Id not Name with unlisted!"}), 400
        ...
        for i in data:
            if i["delete_time"]:
                if datetime.strptime(
                        ...
                        "%d-%m-%Y %H:%M:%S") < datetime.now():
                    db.execute(
                        "DELETE FROM clips WHERE clip_url=?",
                        i["clip_url"])
                    data.remove(i)
...
def post_data():
    if request.method == "GET":
        return jsonify(
            {"message": "POST data to this endpoint to create clips"}), 200
    ...
    if not clip_pass:
        db.execute(
            "INSERT INTO clips (clip_url, clip_name, clip_text, is_editable, is_unlisted, clip_time, delete_time) VALUES (?,?,?,0,?,?,?)",
            str(clip_id),
            clip_name,
            clip_text,
            unlist,
        ...
        db.execute(
            "INSERT INTO clips (clip_url, clip_name, clip_text, clip_pwd, is_editable, is_unlisted, clip_time, delete_time) VALUES (?,?,?,?,0,?,?,?)",
            str(clip_id),
            clip_name,
            clip_text,
            pwd,
    ...
    if successStatus:
        return jsonify(
            {"id": str(clip_id), "Message": "Successfully added!"}), 201

...
class TestDatabaseCleanup:
    ...
    def test_database_cleanup_removes_all_data(self, client):
        ...
        db.execute("""
            ...
            VALUES (?, ?)
        """, 'cleanup_test_user', 'password123')
        
        db.execute("""
            INSERT INTO clips (clip_url, clip_name, clip_text, clip_time) 
            VALUES (?, ?, ?, ?)
        """, 'test123', 'Test Clip', 'Test content', '2025-01-01 12:00:00')
    ...
    def test_sqlite_sequence_is_reset(self, client):
        ...
        db.execute("""
            INSERT INTO users (username, password) 
            VALUES (?, ?)
        """, 'seq_test_user1', 'password123')
        
        db.execute("""
            INSERT INTO users (username, password) 
            VALUES (?, ?)
        """, 'seq_test_user2', 'password123')
    ...
    def test_foreign_key_constraints_restored(self, client):
        ...
        clip_result = db.execute("""
            INSERT INTO clips (clip_url, clip_name, clip_text, clip_time) 
            VALUES (?, ?, ?, ?)
        """, 'fk_test_clip', 'FK Test Clip', 'Test content', '2025-01-01 12:00:00')
